...
export function useLongPress(
  {
    ...
    longPressCallback: (e: MouseEvent | TouchEvent) => void;
  },
  duration: number = 500
) {
  ...
  const onPressStart = useCallback(
    (event: MouseEvent | TouchEvent) => {
      ...
      // supplied callbacl.
      timeout.current = setTimeout(() => longPressCallback(event), duration);
    },
  ...
  const cancelTimeout = useCallback((e: MouseEvent | TouchEvent) => {
